...
impl Dashboard {
    ...
    fn render_current_speed(&mut self, frame: &mut Frame, area: Rect) {
        // Get latest network statistics with enhanced error handling
        let stats = match self.bandwidth_collector.collect() {
            Ok(stats) => {
                // Log successful collection for dashboard monitoring
                if log::log_enabled!(log::Level::Debug) {
                    self.bandwidth_collector.log_success_event(&stats, 0.0);
                }
                stats
            }
            Err(e) => {
                // Enhanced error handling for dashboard
                log::warn!("Dashboard bandwidth collection failed: {}", e);
                
                // Log structured error event if it's a BandwidthError
                if let Some(bandwidth_error) = e.downcast_ref::<crate::collectors::bandwidth_collector::BandwidthError>() {
                    self.bandwidth_collector.log_error_event(bandwidth_error, "dashboard_collection");
                }
    ...
    fn render_interface_list(&mut self, frame: &mut Frame, area: Rect) {
        // Get latest network statistics with graceful error handling
        let stats = match self.bandwidth_collector.collect() {
            Ok(stats) => stats,
            Err(e) => {
                // Log error but continue with empty stats for graceful degradation
                log::debug!("Dashboard interface list collection failed: {}", e);
                vec![]
            }
        };
